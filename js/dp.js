...
window.onload = function(){
	...
	radio_text+='<label><input type="radio" name="limit" id="limitn" value="n" checked onclick="chgMode(this.value)">標準權重</label>';
	radio_text+='<label><input type="radio" name="limit" id="limitl" value="l" onclick="chgMode(this.value)">極限權重</label>';
	if(document.getElementsByClassName('prop').length>0) radio_text+='<label><input type="radio" name="limit" id="limitp" value="p" onclick="chgMode(this.value)">屬性分析</label>';
	radio_text+='<br></form>';
