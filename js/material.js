...
var highlight=['星之海','韶顏傾城','格萊斯','冰風戰歌','白櫻戀歌','時光吟遊'];
var highlight_style=['xzh','syqc','gls','bfzg','bylg','sgyy'];
...
function searchSet(setName,showConstructInd,showConsumeInd){//showConsumeInd is dummy now
	...
	var output=table();
	var cell = '<b>套裝：</b>'+ahref(setName,"chgScopeSub2(3,'"+setName+"')")+'('+searchCountry(setName)+')　全'+setCnt+'個部件材料總覽';
	output+=tr(tab(cell,'colspan="3"'));
	if(thisPatternPrice>0||thisPatternPrice_2>0){
		output += tr(tab((thisPatternPrice>0? '設計圖總價：'+thisPatternPrice+'星幣&emsp;':'')+(thisPatternPrice_2>thisPatternPrice? '設計圖總價(含材料)：'+thisPatternPrice_2+'星幣':''),'colspan="3"'));
	}
	output+=tr(tab('','colspan="3"'));
