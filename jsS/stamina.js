...
function calc(){
	...
	document.getElementById('res1_time').innerHTML=usehr1+"時"+usemin1+"分";
	...
	document.getElementById('ap_rec_tmp').innerHTML="會恢復"+ap_recov2_tmp+"體力";
	if(nowap1>0&&ap_recov2_tmp2<=maxap1){
		document.getElementById('ap_rec_tmp').innerHTML+="到"+ap_recov2_tmp2;
	}
	if(ap_recov2>=0){
		document.getElementById('nowap2').innerHTML="體力在"+ap_recov2+"以下就不會溢出";
	}
	else{
		document.getElementById('nowap2').innerHTML="溢出"+abs_recov+"體力&#x1F623;";
	}
}
function maxap(n){
...
window.onload = function(){ 
	...
	document.getElementById('maxap1').value=100;
